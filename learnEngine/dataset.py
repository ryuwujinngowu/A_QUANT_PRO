...
def _apply_reserved_future_schema(df: pd.DataFrame) -> pd.DataFrame:
    """为未来两阶段开盘架构预留 schema 列位；当前仅占位，不参与训练。"""
    if df.empty:
        return df

    df = df.copy()
    for col in _cfg.RESERVED_FUTURE_LABEL_COLS + _cfg.RESERVED_FUTURE_FEATURE_COLS:
...
if __name__ == "__main__":
    ...
    for date in to_process:
        ...
        try:
            ...
            # ---- Step 2: 多策略候选池收集 ----
            # 重要时序约定：本项目实盘通常在 D+1 凌晨运行，此时 Tushare 的“最新完整盘面”实际对应 D 日收盘后数据。
            # 因此这里显式注入 get_daily_kline_data(D)，供策略内部当作“上一完整交易日盘面”使用。
            # 不要轻易改成 daily_df=None；否则会把这条为数据入库时效性设计的对策抹掉。
    ...
    if os.path.exists(OUTPUT_CSV_PATH):
        validated_df = validate_train_dataset(OUTPUT_CSV_PATH)
        ...
        try:
            ...
            spec = write_split_spec(
                ...
                output_path=split_spec_path,
            )
            logger.info(
                f"✅ Split spec 已写出: {split_spec_path}\n"
