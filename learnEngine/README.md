...
- 2026-03-31：新增 `strategies/high_low_switch_ml_strategy.py`（高低切ML候选策略，D-1涨停池，主板+非ST+首/二/三板，无择时逻辑）；修复 `dataset.py` label date bug（label_engine 应传 feature_trade_date 即 D-1，而非 date=D，否则 merge 全 NaN）；改造 `dataset.py` 为真正的多策略循环（sector_heat + high_low_switch，union ts_codes，一次 bundle 构建，各策略 meta 分别 merge）。py_compile 全通过，import smoke test 通过。**待做：D1 运行时验收（生成10行）**。
