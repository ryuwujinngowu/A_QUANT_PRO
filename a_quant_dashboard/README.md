...
### 全局概览（含近期更新）
- 短线策略汇总表：点击策略名 → 弹窗显示最新一日持仓明细（signal_stock_detail）
- 平均日收益柱状图：支持近5日 / 近30日 / 近90日 / 全期 切换
- 累计收益折线图：点击图例可勾选/取消显示单条策略线

### 模型组合对比（新增）
- 汇聚 `model_dip_buy` / `model_open_buy` / `model_surge_buy` 三个 XGBoost 策略
- 展示：综合胜率 KPI、三线累计收益对比、平均收益柱状图、各自逐日柱状图、逐日三列对比表
