"""
模型v5.2信号恐慌低吸买入（ModelDipBuyV52Agent）
================================================
策略逻辑（时序：D-1 日选股，D 日买入）
--------
使用 sector_heat_xgb_v5.2_auc_first.pkl，D 日恐慌下跌时买入：
...
区别于 model_dip_buy
---------------------
model_dip_buy 使用老版模型（sector_heat_xgb_model.pkl，prob 阈值 0.60）。
...
# ── 策略参数（与 model_dip_buy 保持一致）──────────────────────────────────
DIP_PCT      = 0.03     # 触发低吸的开盘跌幅阈值（3%）
WINDOW_START = "09:30"  # 低吸监测窗口开始
WINDOW_END   = "10:30"  # 低吸监测窗口结束（含）


class ModelDipBuyV52Agent(BaseAgent):
    agent_id   = "model_dip_buy_v52"
    agent_name = "模型v5.2信号恐慌低吸买入"
    agent_desc = (
        "使用 sector_heat_xgb_v5.2_auc_first 模型（prob≥0.50），D-1 日生成信号，"
        "D 日 09:30-10:30 内若价格触及开盘价 -3% 则模拟低吸买入。"
        "与 model_dip_buy（老模型）可做横向比较，需重跑历史数据。"
    )
