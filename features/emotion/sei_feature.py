...
HDI_WEIGHTS = {
    ...
    "max_dd_intra":  0.32,  # 日内最大回撤（最重要，权重最大）
    "pullback_abs":  0.18,  # 收盘离日高距离（冲高回落程度）
...
CANDLE_HDI_ADJUST = {
    ...
    0:   0.00,   # 平盘/十字星：中性，不修正 HDI（classify_candle 修复后会产生此值）
...
SEI_PARAMS = {
    "tanh_k":      14,    # 提高基础分斜率，增强中等涨跌幅下的分值灵敏度

    # 调整项系数（单位：分/单位偏离）
    "gap_coeff":   28,    # 缺口贡献：更敏感地区分高开承接 / 低开修复
    "trend_coeff": 22,    # 趋势贡献：单边走势与震荡日拉开差距
    "vwap_coeff":  28,    # VWAP 偏离减分：筹码散乱的票更快降分
    ...
    "seal_bonus_per":    3,    # 每次封板加分
    "break_penalty_per": 4,    # 每次开板减分
    "seal_max_bonus":    8,    # 封板加分上限
    "break_max_penalty": 12,   # 开板减分上限
}
